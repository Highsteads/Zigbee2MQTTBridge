...
def test_on_connect_queues_subscribed_list_without_logging(plugin, plugin_mod, monkeypatch, helpers_mod):
    # The callback runs on the paho thread — it must queue, never call log()
    # (which is indigo.server.log under the bonnet).
    logged = []
    monkeypatch.setattr(helpers_mod, "log", lambda *a, **k: logged.append(a))
    client = _FakeClient()
...
def test_bridge_devices_wrong_type_warns_and_keeps_cache(plugin, plugin_mod, monkeypatch, helpers_mod):
    warnings = []
    monkeypatch.setattr(
        helpers_mod, "log",
...
def test_bridge_info_wrong_type_warns(plugin, plugin_mod, monkeypatch, helpers_mod):
    warnings = []
    monkeypatch.setattr(
        helpers_mod, "log",
        lambda msg, level="INFO": warnings.append(msg) if level == "WARNING" else None)
